    ...
    Selectik.prototype = {
		...
		_getList: function(e){
			...
			if (e.refreshSelect){
				...
				html = '<span class="custom-text" id="custom-text-title-'+this.$selected[0].text.toUpperCase().split(' ').join('_')+'">'+this.$selected[0].text.toUpperCase()+'</span><div class="select-list '+scrollClass+'">'+scrollHtml+'<ul>'+html+'</ul></div>';
				$(html).prependTo(this.$container);
			}
		...
		_generateHtml: function(){
			...
			for (var i = 0; i < this.$collection.length; i++){
				...
				{
					html += '<li id="filter_option_label_'+ this_value +'" '+($this[0].className == 'selectik-filter-region' ? 'onclick="find_survey_dynamic_select('+this_value+')" data-type="region"' : 'onclick="filter_option('+this_value+')" data-type="filter"')+' data-value="'+this_value+'">'+($this.data('selectik') ? $this.data('selectik') : $this[0].text)+'</li>';
				}
				else{
