    ...
    function compare_chart(data)
    {
        ...
        for (var wave in data) {
           ...
           for (var prop in obj) {
              ...
              if(obj.hasOwnProperty(prop) && (typeof obj[prop].color != 'undefined')){
                // Get Cycle name
                if (obj[prop].cycle_type == 0) {
                    var baseline_title = obj[prop].cycle;
                    var amount = obj[prop].cycle_type == 0 ? obj[prop].amount : 0;
                    baseline_list.push({ y: amount, label: obj[prop].answer});
                }
                if (obj[prop].cycle_type == 1) {
                    var endline_title = obj[prop].cycle;
                    endline_list.push({ y: obj[prop].amount, label: obj[prop].answer});
                };
                color_set.push(obj[prop].color);
              }
           }
        }
        ...
        {
            title:{
              text: "Perbandingan hasil tahun 2013 dan 2014"
            },
            colorSet: "greenShades",
            data: [
            {
              type: "bar",
              showInLegend: true,
              legendText: baseline_title,
              dataPoints: baseline_list
            },
            {
              type: "bar",
              showInLegend: true,
              legendText: endline_title,
